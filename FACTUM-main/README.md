...
*   Access to a Slurm workload manager to run the provided batch scripts.
...
Before running the code, you must verify that the file paths in our scripts match your local machine's setup. Please open the following two files and edit the paths as needed:

1. ``citation_token_detect_long.py``
2. ``citation_token_detect_long.sh``
...
- The output directory where you want to store the results. We stored our results of the scores here: ``~/FACTUM-main/scale/results/llama3.1-8b_neuclir24`` and here: ``~/FACTUM-main/scale/results/llama3.2-3b_neuclir24``. They were too big to be stored here on GitLab so the folders are empty now. 

### Step 4: Execute the Scoring Job
Once the environment is set up and the paths are configured, you can run the main scoring pipeline using the provided Slurm batch script.
...
- ``analysis10/``: This directory contains the output folders for the different experiments conducted using the ``analysis.py`` script. Each subfolder corresponds to a specific analysis run.
- ``plots/``: This folder contains all the plots and figures generated by the ``plotting.py`` script, providing visual representations of the analysis results.
